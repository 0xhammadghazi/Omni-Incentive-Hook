...
1. The example hook [OmniIncentiveHook.sol](src/OmniIncentiveHook.sol) demonstrates the `beforeSwap()` and `afterSwap()` hooks
2. The test template [OmniIncentiveHook.t.sol](test/OmniIncentiveHook.t.sol) preconfigures the v4 pool manager, test tokens, and test liquidity.
...
```
forge script script/00_OmniIncentiveHook.s.sol \
